...
# for loop to add seats 15% vip, 25% regular, 60% economy to all matches based on number of seats
for match in read_all(matches_db):
    ...
    for i in range(vip_seats):
        add_record(seats_db, seats_fields, {'seat_id': f'{i+1}', 'match_id': match_id, 'catagory': 'VIP', 'occupied': 'False'})
    for i in range(regular_seats):
        add_record(seats_db, seats_fields, {'seat_id': f'{i+1}', 'match_id': match_id, 'catagory': 'Regular', 'occupied': 'False'})
    for i in range(economy_seats):
        add_record(seats_db, seats_fields, {'seat_id': f'{i+1}', 'match_id': match_id, 'catagory': 'Economy', 'occupied': 'False'})
